...
# class Grid:
#     @staticmethod
#     def grid(size, square_size, line_width, color):
#         texture = Texture.create(size=size, colorfmt='rgb')
#         buf = []
#         L = [i for i in range(line_width)]
#         for y in range(int(size[1]) + 1):
#             for x in range(int(size[0]) + 1):
#                 if x % square_size[0] in L or y % square_size[1] in {0, 1, 2}:
#                     buf.extend([int(v * 255) for v in color])
#                 else:
#                     buf.extend([int(255)] * 3)
